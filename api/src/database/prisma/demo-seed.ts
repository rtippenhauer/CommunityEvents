...
async function seedLocations(
  ...
  for (const venue of VENUES) {
    ...
    //
    // `filePath` holds a short path exactly as a real uploaded photo does, so
    // the card component needs no special case; the art itself is generated by
    // the route from this slug. Nothing is written to disk, which is why a
    // deleted demo leaves nothing behind to clean up.
    await prisma.location_photos.create({
      data: {
        tenantId,
        locationId: location.id,
        filePath: `/api/v1/demo/venue-art/${venueSlug(venue.name)}`,
        fileName: `${venueSlug(venue.name)}.svg`,
