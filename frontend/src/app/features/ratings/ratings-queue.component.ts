...
@Component({
  ...
  template: `
    <div class="ratings-container">
      ...
      <div class="ratings-controls">
        ...
        <mat-slide-toggle [checked]="showRated()" (change)="showRated.set($event.checked)" class="rated-toggle">
          Show already rated
        </mat-slide-toggle>
      </div>
      ...
      @if (loading()) {
        ...
        <mat-card class="empty-card">
          <mat-card-content>
            ...
            @if (showRated()) {
              ...
              <button class="show-all-link" (click)="showRated.set(true)">Show already rated visits</button>
            }
          </mat-card-content>
        </mat-card>
      } @else {
        <div class="queue-list">
          @for (item of filtered(); track item.eventId) {
            <div class="queue-item" [class.already-rated]="item.alreadyRated" (click)="goToRestaurant(item.restaurantId)">
              ...
              <div class="item-info">
                ...
                <span class="item-date">{{ (item.eventDate + 'T12:00:00') | date:'MMMM d, y' }}</span>
              </div>
              <div class="item-status">
                @if (item.alreadyRated) {
                  <mat-chip class="chip-rated">
                    <mat-icon>star</mat-icon> Rated
                  </mat-chip>
                } @else {
  ...
  styles: [`
    ...
    .ratings-header { margin-bottom: 20px; }
    .ratings-title { margin: 0 0 4px; font-size: 1.4rem; }
    .ratings-subtitle { margin: 0; color: #888; font-size: 0.9rem; }
    .ratings-controls {
      display: flex;
      align-items: center;
      gap: 16px;
    ...
    .rated-toggle { white-space: nowrap; }
    ...
    .empty-icon { font-size: 3rem; width: 3rem; height: 3rem; color: #ddd; }
    .show-all-link {
      background: none;
      border: none;
      color: var(--db-amber);
    ...
    .queue-item {
      ...
      &.already-rated { background: #fafafa; }
    }
    .item-photo {
      width: 56px;
      height: 56px;
    ...
    .chip-rated { background: #c8e6c9 !important; color: #2e7d32 !important; }
    .chip-pending { background: #fff9c4 !important; color: #7a6200 !important; }
    .item-chevron { color: #ccc; flex-shrink: 0; }
  `],
})
