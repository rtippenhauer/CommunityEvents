...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, RouterLink, MatButtonModule, MatCardModule, MatChipsModule, MatIconModule, MatProgressSpinnerModule],
  template: `
  ...
  `,
  styles: [`
    .page-header { margin-bottom: 24px; h1 { margin: 0; font-size: 1.75rem; color: var(--db-brown-dark); } }
    .center { display: flex; justify-content: center; padding: 48px; }
    .empty { text-align: center; color: #999; padding: 48px 0; }
    .ann-list { display: flex; flex-direction: column; gap: 12px; max-width: 720px; }
    .ann-card {
      display: block;
      background: #fff;
      ...
      &:hover { box-shadow: 0 4px 16px rgba(61,28,5,0.12); transform: translateY(-1px); }
    }
