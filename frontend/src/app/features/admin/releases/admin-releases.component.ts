...
@Component({
        ...
        <section class="create-panel">
          ...
          <form [formGroup]="form" (ngSubmit)="save()" class="release-form">
            ...
            <mat-form-field appearance="outline">
              ...
              <mat-error>
                @if (form.controls.version.hasError('pattern')) { Must be valid semver (e.g. 1.2.0) }
                @else { Version is required }
              </mat-error>
            </mat-form-field>

            <mat-form-field appearance="outline">
              <mat-label>Title</mat-label>
              <input matInput formControlName="title" placeholder="Brief release headline" maxlength="200" />
            ...
            <div class="feedback-linker">
              <div class="linker-header">
                ...
                @if (linkedIds().size > 0) {
                  <button mat-button type="button" class="refresh-btn" (click)="refreshNotes()" matTooltip="Overwrite notes with bullet list from linked items">
                ...
                <div class="feedback-checkboxes">
                  @for (fb of resolvedFeedback(); track fb.id) {
                    <label class="fb-checkbox-row" [class.checked]="isLinked(fb.id)">
                      <input type="checkbox" [checked]="isLinked(fb.id)" (change)="toggleLink(fb.id)" />
                      <span class="fb-row-content">
                        <span class="fb-cat" [style.color]="catColor(fb.category)">{{ catLabel(fb.category) }}</span>
  ...
  `,
  styles: [`
    .releases-page { max-width: 1000px; margin: 0 auto; padding: 24px 16px; }
    .page-header { display: flex; align-items: center; gap: 8px; margin-bottom: 24px; h1 { margin: 0; font-size: 1.75rem; color: var(--db-brown-dark); } }
    ...
    .release-form { display: flex; flex-direction: column; gap: 20px; }
    ...
    .quill-label { display: block; font-size: 0.75rem; color: rgba(0,0,0,0.6); padding: 8px 12px 0; }
    .release-quill { display: block; }
    ::ng-deep .release-quill .ql-container { border: none; min-height: 180px; font-size: 0.95rem; }
    ::ng-deep .release-quill .ql-toolbar { border: none; border-bottom: 1px solid rgba(0,0,0,0.12); }

    .feedback-linker { border: 1px solid #eee; border-radius: 8px; padding: 16px; }
    .linker-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
    .feedback-linker h3 { margin: 0; font-size: 0.9rem; color: #555; text-transform: uppercase; letter-spacing: 0.05em; }
    .refresh-btn { font-size: 0.8rem; color: #888; mat-icon { font-size: 0.9rem; width: 0.9rem; height: 0.9rem; } }
    .empty-resolved { font-size: 0.85rem; color: #aaa; margin: 0; }
    .feedback-checkboxes { display: flex; flex-direction: column; gap: 8px; max-height: 260px; overflow-y: auto; }
    .fb-checkbox-row {
      display: flex; align-items: flex-start; gap: 8px; padding: 8px 10px; border-radius: 6px;
      cursor: pointer; transition: background 0.1s;
      &:hover { background: #f5f5f5; }
      &.checked { background: #e3f2fd; }
      input { margin-top: 2px; flex-shrink: 0; cursor: pointer; }
    }
    .fb-row-content { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; font-size: 0.85rem; }
    .fb-cat { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; }
    .fb-title { color: #333; }
    ...
    .credit-preview {
      ...
      border-radius: 8px; padding: 10px 14px; font-size: 0.85rem; color: #555;
      mat-icon { font-size: 1rem; width: 1rem; height: 1rem; color: var(--db-blue, #1E4D8C); flex-shrink: 0; margin-top: 1px; }
    }
    ...
    .release-row {
      ...
      &.draft { border-color: #ffe082; background: #fffde7; }
    }
    .release-row-header { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 4px; }
    .version-badge {
      font-size: 0.75rem; font-weight: 800; padding: 2px 8px; border-radius: 10px;
      background: var(--db-blue, #1E4D8C); color: white;
    }
    .pub-date { font-size: 0.8rem; color: #aaa; }
    .draft-badge {
...
export class AdminReleasesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.releasesService.getResolvedFeedback().subscribe({
      next: (data) => { this.resolvedFeedback.set(data); this.loadingFeedback.set(false); },
      error: () => this.loadingFeedback.set(false),
    });
    this.releasesService.getAll().subscribe({
      next: (data) => { this.releases.set(data); this.loadingReleases.set(false); },
  ...
  isLinked(id: number): boolean { return this.linkedIds().has(id); }
  ...
  catColor(c: string): string {
    const map: Record<string, string> = { bug: '#c62828', feature_request: '#1565c0', comment: '#555' };
    return map[c] ?? '#555';
  }
  ...
  publish(release: Release): void {
    ...
    this.releasesService.publish(release.id).subscribe({
      next: (updated) => {
        ...
        this.snackBar.open(`v${updated.version} published!`, 'View', { duration: 4000 })
          .onAction().subscribe(() => void this.router.navigate(['/updates']));
      },
      error: () => {
        this.publishingId.set(null);
