...
@Component({
  ...
  template: `
    <div class="board-page">
      ...
      <div class="sort-bar">
        <div class="sort-left">
          <span class="result-count">{{ visibleItems().length }} item{{ visibleItems().length === 1 ? '' : 's' }}</span>
          @if (hiddenCount() > 0) {
            <button mat-button class="completed-toggle" (click)="showCompleted.set(!showCompleted())">
        ...
        <div class="sort-buttons">
          <button mat-button [class.active-sort]="sort() === 'newest'" (click)="setSort('newest')">Newest</button>
          <button mat-button [class.active-sort]="sort() === 'upvotes'" (click)="setSort('upvotes')">Most Upvoted</button>
        </div>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="empty-state">
          ...
          <p>{{ hiddenCount() > 0 ? 'All items are completed.' : 'No feedback yet in this category.' }}</p>
          @if (hiddenCount() > 0) {
            <button mat-stroked-button (click)="showCompleted.set(true)">Show completed items</button>
          } @else {
            <button mat-raised-button color="primary" routerLink="/feedback/new">Be the first to submit</button>
          }
        </div>
      } @else {
        <div class="feedback-list">
          @for (item of visibleItems(); track item.id) {
            <a class="feedback-row" [routerLink]="['/feedback', item.id]" [class.completed-row]="isCompleted(item.status)">
              <!-- Upvote -->
              <button
                class="upvote-btn"
              ...
              <div class="row-content">
                <div class="row-top">
                  ...
                  @if (item.isPrivate) {
                    <mat-icon class="private-icon" matTooltip="Private — only you and admins can see this">lock</mat-icon>
                  }
                </div>
  ...
  `,
  styles: [`
    .board-page { max-width: 800px; margin: 0 auto; padding: 24px 16px; }
    ...
    .active-sort { font-weight: 700; color: var(--db-blue, #1E4D8C); }
    ...
    .feedback-list { display: flex; flex-direction: column; gap: 2px; }
    ...
    .row-top { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 4px; }
    ...
    .row-title { font-size: 0.95rem; font-weight: 600; color: #222; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .row-meta { font-size: 0.8rem; color: #999; display: flex; gap: 6px; }
    .dot { color: #ccc; }

    .arrow-icon { color: #ccc; flex-shrink: 0; }
  `],
})
export class FeedbackBoardComponent implements OnInit {
  ...
  readonly tabCategories: (FeedbackCategory | undefined)[] = [undefined, 'bug', 'feature_request', 'comment'];
  ...
  isCompleted(status: string): boolean { return COMPLETED_STATUSES.has(status); }
  releaseVersion(feedbackId: number): string | undefined { return this.releaseMap().get(feedbackId); }
  ...
  catLabel(c: string): string { return CATEGORY_LABELS[c as FeedbackCategory] ?? c; }
  catColor(c: string): string { return CATEGORY_COLORS[c as FeedbackCategory] ?? '#555'; }
}
