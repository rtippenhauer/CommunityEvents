...
@Component({
        ...
        @if (submitted()) {
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" class="feedback-form">
              ...
              <mat-select formControlName="category">
                <mat-select-trigger>
                  @switch (form.controls.category.value) {
                    @case ('bug') { <span class="opt-row"><mat-icon class="opt-icon bug-icon">bug_report</mat-icon> Bug Report</span> }
                    @case ('feature_request') { <span class="opt-row"><mat-icon class="opt-icon feature-icon">lightbulb</mat-icon> Feature Request</span> }
                    @case ('comment') { <span class="opt-row"><mat-icon class="opt-icon comment-icon">chat_bubble_outline</mat-icon> General Comment</span> }
                  }
                </mat-select-trigger>
                <mat-option value="bug">
                  <span class="opt-row"><mat-icon class="opt-icon bug-icon">bug_report</mat-icon> Bug Report</span>
                </mat-option>
                <mat-option value="feature_request">
                  <span class="opt-row"><mat-icon class="opt-icon feature-icon">lightbulb</mat-icon> Feature Request</span>
                </mat-option>
                <mat-option value="comment">
                  <span class="opt-row"><mat-icon class="opt-icon comment-icon">chat_bubble_outline</mat-icon> General Comment</span>
                </mat-option>
              </mat-select>
            ...
            <mat-form-field appearance="outline">
              ...
              <input matInput formControlName="title" placeholder="Brief summary of the issue or idea" maxlength="200" />
              <mat-hint align="end">{{ form.controls.title.value.length }} / 200</mat-hint>
              <mat-error>
                @if (form.controls.title.hasError('minlength')) { At least 3 characters required }
  ...
  `,
  styles: [`
    .new-page { max-width: 680px; margin: 32px auto; padding: 0 16px; }
    .new-card {
      background: white;
      border-radius: 12px;
      padding: 32px;
    ...
    .card-header { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; h1 { margin: 0; font-size: 1.5rem; color: var(--db-brown-dark); } }
    .back-btn { margin-left: -8px; }
    .subtitle { margin: 0 0 24px; font-size: 0.9rem; color: #666; }
    ...
    .quill-wrapper {
      ...
      &.quill-error { border-color: #f44336; }
    }
    .quill-label {
      display: block;
      font-size: 0.75rem;
    ...
    ::ng-deep .quill-editor .ql-container { border: none; font-size: 0.95rem; min-height: 160px; }
    ::ng-deep .quill-editor .ql-toolbar { border: none; border-bottom: 1px solid rgba(0,0,0,0.12); }
    .quill-err-msg { font-size: 0.75rem; color: #f44336; padding: 4px 12px 8px; }

    .private-toggle { display: flex; align-items: center; }
