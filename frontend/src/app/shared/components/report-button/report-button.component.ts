...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <p class="report-info">
        Report this to the moderation team. Moderators will review it and take action if it violates
...
export class ReportButtonComponent {
  ...
  openDialog(): void {
    this.dialog
      .open<ReportDialogComponent, void, ReportDialogResult>(ReportDialogComponent, {
        width: '400px',
      })
      .afterClosed()
      .subscribe((result) => {
        ...
        this.reportsService.submit(this.contentType(), this.contentId(), result.reason).subscribe({
          next: () => {
            ...
            this.snackBar.open('Report submitted — moderators will review it.', 'OK', {
              duration: 4000,
            });
          },
          error: (err) => {
            ...
            if (msg?.includes('already reported')) {
              this.reported.set(true);
              this.snackBar.open('You have already reported this content.', 'OK', {
                duration: 3000,
              });
            } else {
