...
@Component({
    ...
    @if (loading()) {
      ...
      <div class="card-inner">
        <a [routerLink]="['/members', member()!.id]" class="card-avatar">
          <img [src]="member()!.profilePhotoPath ?? '/avatars/bear-default.jpg'"
               [alt]="member()!.fullName" />
        </a>
        <div class="card-body">
          <a [routerLink]="['/members', member()!.id]" class="card-name">{{ member()!.fullName }}</a>
          ...
          <div class="card-badges">
            @if (member()!.hasFacebook) {
              @if (member()!.facebookProfileUrl) {
                <a [href]="member()!.facebookProfileUrl!" target="_blank" rel="noopener noreferrer"
                   class="provider-badge badge-fb" title="View Facebook profile">Facebook</a>
              } @else {
            ...
            @if (member()!.googleEmail) {
              <a [href]="'https://mail.google.com/mail/?view=cm&fs=1&to=' + member()!.googleEmail"
                 ...
                 class="provider-badge badge-g" [title]="member()!.googleEmail!">Gmail</a>
            }
          </div>
        </div>
      </div>
  ...
  styles: [`
    ...
    .card-loading { display: flex; justify-content: center; padding: 12px; }
    ...
    .card-email {
      ...
      overflow: hidden;
      text-overflow: ellipsis;
      &:hover { color: #1E4D8C; text-decoration: underline; }
    }
