...
3. Create and check out a new branch off `main` named `phase-<number>-<kebab-case-slug>` (e.g. `phase-25-angular-19-22-upgrade`), derived from the phase description. Do not push it yet — it's pushed for the first time in `/phase-testing`, when the phase is ready to go to stage.
4. Report back the branch name and confirm it's checked out, and note that the phase ends with `/phase-testing` (stage + Rob's testing pass) and then `/phase-done` (merge).
