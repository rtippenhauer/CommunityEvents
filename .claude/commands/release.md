...
2. Read `docs/NEXT_RELEASE.md` as the starting draft (it accumulates entries from `/phase-done` and ad hoc work between releases). Fold in anything from this session that isn't reflected there yet. Turn it into customer-facing release note copy (title + body). Write copy using the `{{points}}`/`{{locations}}`/`{{events}}` placeholder tokens instead of hardcoded DinnerBears wording (e.g. "the {{points}} leaderboard", not "the Bear Points leaderboard") — the shared note ships to every fork and these get substituted per-instance at render time (see `BrandConfigService`/`updates.component.ts`). Show it to Rob and get his approval/edits before continuing — don't commit it until he's signed off.
3. Once approved, write it to a new file `api/release-notes/$ARGUMENTS.md`:
   ```
   # <Title>
