...
Phase 29 (white-label template for a new group) is in progress on branch `phase-29-white-label-template`. Rob has resolved its four open questions (restaurants-vs-locations, address privacy, attendance migration, event-cadence default) and all four are now implemented and verified end-to-end (real e2e tests against a booted app + migrated MySQL schema, both workspaces typecheck clean, `ng build` succeeds): the `restaurant`→`location` identifier rename (UI copy still says "Restaurant"); per-location privacy (address hidden until a member RSVPs "Going", admin/mod always see it, `app_config`-driven default for new locations, overridable per location, new `/admin/settings` screen); and the event-cadence default (new-event form now reads its default weekday/time from `app_config` instead of a hardcoded next-Tuesday-6:30pm). Attendance needed no code — Sons just starts fresh with their next event, no historical import. Branding is also now done: app name/tagline/3 core colors are `app_config`-backed and admin-editable at `/admin/settings`, applied live via a new `BrandConfigService` — including bridging Angular Material's own `color="primary"`/`"accent"` components (buttons, toggles, checkboxes, form fields — not just hand-styled elements) to the admin's chosen colors via Material's `--mat-sys-*` system tokens. The phase's remaining scope is now also complete (2026-07-24) and verified end-to-end against a disposable local instance: admin-uploadable brand images (logo/splash/icon slots, with compiled-in-default fallback and live favicon swap); single-region UX (`CityService` sole-active-city fallback + hiding the city selector/filter across browse pages and auto-selecting it in create/edit forms); the `dinnerbears.com` hardcoding audit (contact emails now derive from `BASE_DOMAIN` via `common/config/instance-contact.ts` with env overrides — OAuth/CORS needed nothing); an idempotent env-driven bootstrap (`api/src/bootstrap.ts` → `dist/bootstrap.js`, with a guardrail) that seeds a fork's one city, branding, email-config row, and first password admin; and `docs/NEW_INSTANCE_SETUP.md`. The one carried-forward deferral is monthly "Nth weekday" event cadence ("2nd Saturday") — configurable cadence still covers a fixed weekly day/time only, pending Sons' confirmed schedule. Rob wants to stand up a second, separately-branded, single-region community dining site (Southwest Ohio, group name "Sons") as a fork of this codebase — own domain, database, secrets, and branding, not another DinnerBears city. See PHASES.md for full scope and definition of done.
