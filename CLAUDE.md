...
Phase 29 (white-label template for a new group) is in progress on branch `phase-29-white-label-template`. Rob has resolved its open questions (restaurants-vs-locations, address privacy, attendance migration, event-cadence default; see PHASES.md), and the first one — renaming `restaurant`/`Restaurant` to `location`/`Location` throughout the DB schema, API, and frontend identifiers, while UI copy keeps saying "Restaurant" — is now implemented and verified (migrations tested up/down against seeded data, both workspaces typecheck clean, `ng build` succeeds, e2e passes). The other three decisions (location privacy, attendance-in-app, event-cadence config) and the rest of Phase 29's scope (branding extraction, bootstrap script, setup docs) are still unimplemented. Rob wants to stand up a second, separately-branded, single-region community dining site (Southwest Ohio, group name "Sons") as a fork of this codebase — own domain, database, secrets, and branding, not another DinnerBears city. See PHASES.md for full scope and definition of done.
