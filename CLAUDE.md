...
Phase 29 (white-label template for a new group) is in progress on branch `phase-29-white-label-template`. Rob has resolved its four open questions (restaurants-vs-locations, address privacy, attendance migration, event-cadence default) and three of the four are now implemented and verified end-to-end (real e2e tests against a booted app + migrated MySQL schema, both workspaces typecheck clean, `ng build` succeeds): the `restaurant`→`location` identifier rename (UI copy still says "Restaurant"); per-location privacy (address hidden until a member RSVPs "Going", admin/mod always see it, `app_config`-driven default for new locations, overridable per location, new `/admin/settings` screen); and the event-cadence default (new-event form now reads its default weekday/time from `app_config` instead of a hardcoded next-Tuesday-6:30pm). Attendance needed no code — Sons just starts fresh with their next event, no historical import. Still unimplemented: the rest of Phase 29's scope (branding extraction, bootstrap script, `NEW_INSTANCE_SETUP.md`, `dinnerbears.com` hardcoding audit). Rob wants to stand up a second, separately-branded, single-region community dining site (Southwest Ohio, group name "Sons") as a fork of this codebase — own domain, database, secrets, and branding, not another DinnerBears city. See PHASES.md for full scope and definition of done.
