...
Phase 30 (editable legal copy) is in progress on branch `phase-30-editable-legal-copy` — split out of Phase 29 to do first, since it stands on its own. Terms/Privacy/About currently live as hardcoded HTML in Angular components; this phase moves them into `app_config` rows with an admin editor (reusing the existing `ngx-quill` component from feedback/releases admin screens) so they're editable without a code change or redeploy. See PHASES.md for full scope and definition of done.

Phase 29 (white-label template for a new group) is parked mid-planning on branch `phase-29-white-label-template` until Phase 30 wraps. Rob wants to stand up a second, separately-branded, single-region community dining site (Southwest Ohio, group name "Sons") as a fork of this codebase — own domain, database, secrets, and branding, not another DinnerBears city. See PHASES.md for full scope, definition of done, and open questions (restaurants-vs-locations, address privacy, Sons' Google-Doc-to-our-attendance migration, the hardcoded weekly-Tuesday event default).
