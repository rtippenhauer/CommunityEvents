...
There's also an open ad hoc bugfix branch `bugfix-hide-unconfigured-facebook-login` (not yet merged) that nulls out the hardcoded Facebook App ID on prod, since the Facebook app isn't fully set up (Go-Live checklist incomplete) and real members were seeing a broken "Continue with Facebook" button.
